...
for path in dirs:
    hf.endureAndCreateDirectory(path)
...
def initadxl345():
    ...
    initadxl = ([0x2D, 0b00000000], [0x2E, 0b00000000],
                [0x2F, 0b00000000], [0x2C, 0b00001100],
                [0x31, 0b00000010], [0x2D, 0b00001000],
                [0x38, 0b01000101])
    for x in initadxl:
        spi.xfer2(x)
...
while True:
    ...
    while buffer_counter < 5:
        ...
        if ((axia[0] > c.POSITIVE_TRIGGER_THRESHOLD or
             axia[0] < c.NEGATIVE_TRIGGER_THRESHOLD or
             axia[1] > c.POSITIVE_TRIGGER_THRESHOLD or
             axia[1] < c.NEGATIVE_TRIGGER_THRESHOLD or
             axia[2] > c.POSITIVE_TRIGGER_THRESHOLD or
             axia[2] < c.NEGATIVE_TRIGGER_THRESHOLD) and trigger == 0):
            trigger = 1
            counter = 0
            GPIO.output(c.BLUE_LED, True)
    ...
    if counter > c.COLLECTION_STOP:
        ...
        hf.endureAndCreateDirectory(c.ABSOLUTE_PATH + 'raw/{0}_{1}_{2}'.format(event_timestamp.year,
                                                                               event_timestamp.month, 
                                                                               event_timestamp.day))
        hf.endureAndCreateDirectory(c.ABSOLUTE_PATH + 'fixed/{0}_{1}_{2}'.format(event_timestamp.year,
                                                                                 event_timestamp.month, 
                                                                                 event_timestamp.day))
